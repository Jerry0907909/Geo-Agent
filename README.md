...
Geo-Agent 已从单纯的 `Chat + RAG Demo` 演进为一个具备正式 `Agent Runtime` 的地质智能问答系统，支持普通对话、显式 RAG 检索、多步 Agent 任务执行与流式过程展示。
...
- `P0-P9` 已基本完成
- 已具备 `chat / rag / agent` 三条正式链路
- 已具备 `/api/agent/*`、SSE 事件流、前端 `agent` 模式接入
- 已完成 DeepSeek 风格的主界面、系统设置、登录页、文献管理页视觉统一
- 当前主剩余项为持续性的可靠性细节打磨、测试覆盖补强与部署文档完善

三条模式的当前定义：

- `chat`：普通对话模式，适合直接问答
- `rag`：显式文献检索模式，适合基于知识库做定向问答
- `agent`：正式单 Agent 模式，适合需要规划、分步调用工具、展示执行过程的复杂任务

当前前端交互方向：

- 普通问答与文献检索保持轻量、低装饰的 DeepSeek 风格正文密度
- Agent 模式采用工作区式布局，右侧过程面板支持收缩
- 文献检索模式提供本地、外部、混合三种检索入口，并带有平滑模式切换与功能区展开动画
- 系统设置提供通用设置、账号管理、数据管理、服务协议四个可用入口
...
- **🧠 单 Agent 执行**
  - 支持根据用户任务生成结构化计划
  - 支持按步骤调用工具并输出中间 observation
  - 支持流式展示 `plan / step_start / tool_call / tool_result / replan / final`
  - 支持运行取消、run 恢复与来源聚合展示
...
- **🎨 主题切换**：支持亮色/暗色主题
- **📱 响应式设计**：适配桌面端和移动端
- **🧩 系统设置**：支持用户偏好、账号资料、密码修改、搜索历史与知识库维护入口
...
│                    Runtime / RAG / Tool 层                   │
│ ┌────────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐ │
│ │AgentRuntime│ │ Planner  │ │ Executor │ │ ToolRegistry │ │
│ └────────────┘ └──────────┘ └──────────┘ └──────────────┘ │
│ ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│ │LLM提供商 │  │RAG链     │  │文档加载  │  │Web Search│   │
│ └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
└─────────────────────────────────────────────────────────────┘
                            ↕
┌─────────────────────────────────────────────────────────────┐
...
3. **RAG 链**：检索增强生成流程
4. **Agent Runtime**：统一编排 Planner、Executor、Synthesis 与事件流
5. **Tool Registry**：统一暴露 `literature_search / knowledge_query / web_search / document_metadata / calculator`
6. **ChromaDB 管理器**：向量数据库操作
7. **文档加载器**：多格式文档解析
8. **文本分割器**：智能文本分块
...
```bash
git clone git@github.com:Jerry0907909/Geo-Agent.git
cd Geo-Agent
```
...
- `POST /api/agent/query` - 非流式 Agent 查询
- `POST /api/agent/stream` - 流式 Agent 查询
- `GET /api/agent/runs/{run_id}` - 获取 Agent 运行状态
- `POST /api/agent/runs/{run_id}/cancel` - 取消 Agent 运行
...
在文献检索模式中选择 `外部` 或 `混合`。`本地` 仅使用本地知识库，`外部` 仅使用网页结果，`混合` 会融合本地知识库与外部网页结果。
