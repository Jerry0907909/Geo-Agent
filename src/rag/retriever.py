...
只输出查询变体，每行一个，不要编号或其他说明。"""),
            ...
            result = chain.invoke({"query": query, "num": num_variations})
    ...
    def retrieve(self, query: str, top_k: Optional[int] = None, min_relevance_score: float = 0.0, use_multi_query: bool = True, search_all: bool = True) -> List[Document]:
        """执行增强检索（LangChain v1 风格）
        
        使用 MultiQuery 策略生成多个查询变体，合并结果后去重排序。
        
        Args:
            query: 查询文本
            top_k: 返回文档数量
            min_relevance_score: 最小相关度阈值 (0.0-1.0)
            use_multi_query: 是否使用多查询策略
        ...
        # 生成查询变体
        if use_multi_query:
            queries = self._generate_query_variations(query, num_variations=2)
        else:
            queries = [query]
        ...
        for q in queries:
            ...
            if search_all:
                # 跨所有知识库搜索
                docs = search_all_collections(query=q, top_k=k * 2)
            else:
                # 只搜索默认知识库
                docs = self.vector_store.similarity_search(query=q, top_k=k * 2)
    ...
    def run(self, question: str, top_k: Optional[int] = None, min_relevance_score: float = 0.0) -> RAGResult:
        """完整 RAG 流程：检索 + 生成"""
        # 1. 检索文档
    ...
    def stream(self, question: str, top_k: Optional[int] = None, min_relevance_score: float = 0.0) -> tuple[List[Document], Iterator[str]]:
    ...
    async def astream(self, question: str, top_k: Optional[int] = None, min_relevance_score: float = 0.0) -> tuple[List[Document], Any]:
